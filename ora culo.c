    ...
    if(opc == 0){
        do{
            if(turnos % 2 == 0 || turnos == 0){
                ...
                sprintf(buffer1, "jugada_gato.exe tablero.txt %c %c", piezaalazar[0], piezaalazar[1]);
                system(buffer1);
                //printf("%s", buffer1);
            }
            else{
                printf("\nEl jugador 2 realizo el tiro, se muestra el tablero a continuacion:\n\n");
                sprintf(buffer2, "jugada_gato.exe tablero.txt %c %c", piezaalazar[1], piezaalazar[0]);
                system(buffer2);
                //printf("%s", buffer2);
            }
